...
public class AircraftManagementDatabase extends Observable{
	 ...
	 */
	private ManagementRecord[] MRs = new ManagementRecord[maxMRs];

	//Initialize all the ManagementRecord in the array
	...
	//Forward a status change request to the MR given by the mCode supplied as a parameter. Parameter newStatus is the requested new status. No effect is expected if the current status is not a valid preceding status. 
	//This operation is appropriate when the status change does not need any additional information to be noted. It is present instead of a large collection of public operations for requesting specific status changes.
	...
	public void radarDetect(FlightDescriptor fd){
		...
		for (int i = 0; i <= MRs.length; i++){
			if (MRs[i].getStatus() == ManagementRecord.FREE){
				MRs[i].radarDetect(fd,i);
				setChanged();
				notifyObservers();
				return;
			}
	...
	/***The given passenger is boarding the aircraft with the given mCode. Forward the message to the given MR for recording in the LSH passenger list.*/
